export const dom = {
  authForm: document.querySelector("#authForm"),
  keyInput: document.querySelector("#keyInput"),
  authMessage: document.querySelector("#authMessage"),
  dashboard: document.querySelector("#dashboard"),
  securityWarning: document.querySelector("#securityWarning"),
  statusDot: document.querySelector("#statusDot"),
  statusText: document.querySelector("#statusText"),
  clientCount: document.querySelector("#clientCount"),
  clientMetricButton: document.querySelector("#clientMetricButton"),
  analyticsPanel: document.querySelector("#analyticsPanel"),
  analyticsToday: document.querySelector("#analyticsToday"),
  analyticsMonth: document.querySelector("#analyticsMonth"),
  analyticsYear: document.querySelector("#analyticsYear"),
  analyticsChart: document.querySelector("#analyticsChart"),
  analyticsBackupButton: document.querySelector("#analyticsBackupButton"),
  analyticsRestoreButton: document.querySelector("#analyticsRestoreButton"),
  analyticsRestoreInput: document.querySelector("#analyticsRestoreInput"),
  analyticsStatus: document.querySelector("#analyticsStatus"),
  playerCount: document.querySelector("#playerCount"),
  roomCount: document.querySelector("#roomCount"),
  adminCount: document.querySelector("#adminCount"),
  updatedAt: document.querySelector("#updatedAt"),
  refreshButton: document.querySelector("#refreshButton"),
  snapshotButton: document.querySelector("#snapshotButton"),
  snapshotMessage: document.querySelector("#snapshotMessage"),
  roomCreateForm: document.querySelector("#roomCreateForm"),
  roomSlugInput: document.querySelector("#roomSlugInput"),
  roomDisplayInput: document.querySelector("#roomDisplayInput"),
  roomDescriptionInput: document.querySelector("#roomDescriptionInput"),
  roomManagerMessage: document.querySelector("#roomManagerMessage"),
  fullBackupDownloadButton: document.querySelector("#fullBackupDownloadButton"),
  serverBackupButton: document.querySelector("#serverBackupButton"),
  fullBackupRestoreButton: document.querySelector("#fullBackupRestoreButton"),
  fullBackupRestoreInput: document.querySelector("#fullBackupRestoreInput"),
  fullBackupStatus: document.querySelector("#fullBackupStatus"),
  previewFrame: document.querySelector("#gamePreview"),
  previewOpen: document.querySelector("#previewOpen"),
  playerOpen: document.querySelector("#playerOpen"),
  previewRoom: document.querySelector("#previewRoom"),
  rooms: document.querySelector("#rooms"),
  emptyTemplate: document.querySelector("#emptyTemplate")
};
